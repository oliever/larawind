...
class ProjectController extends Controller
{
    ...
    public function pdf(Project $project)
    {
        ...
        $data['selectedAffectedAreas'] = array_fill_keys($project->affectedAreas()->pluck('id')->toArray(),'checked');

        $data['departments'] = array_chunk(Department::where(['team_id'=>auth()->user()->currentTeam->id])->get()->toArray(),4);
        $data['selectedDepartments'] = array_fill_keys($project->departments()->pluck('id')->toArray(),'checked');
